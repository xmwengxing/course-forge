...
### 上下左右边距规范
- layout 顶部必须预留 **50-70px** 空间，内容不得紧贴画布上方边缘（`.scene-pad` 已通过 `--stage-pad-y` 提供基础值，章节 CSS 中**禁止用 `padding` 缩写覆盖**，如需增加水平边距请使用独立的 `padding-left/padding-right`）
- layout 底部必须预留 **60-80px** 空间，避免字幕（浮动在底部）与画面内容重叠
- 卡片内边距：水平 **32-40px**，垂直 **28-36px**
- 段间距（gap）：**36-48px**
- 卡片间距（gap）：**20-24px**
- 容器宽度：按布局模式自由发挥——全版式用 `width:100%`，分栏用 `3fr 2fr` 等比，网格用 `repeat()`，不要所有章节都套一个 `max-width:1200px` 的居中盒子
...
### 内容驱动布局选择（禁止所有章节使用同一种布局）

**本章内容属于什么类型 → 就选什么布局模式，严禁全部章节使用居中垂直列。**

画布为 1920×1080 固定舞台，内容不应被限制在居中的 800px 窄带中。大胆利用整个画面。

#### 布局模式速查表（8 种）

| 模式 | 适用内容类型 | CSS 关键词 | 示例 |
|:--|:--|:--|:--|
| **全版式** | 架构图/流程图/大图 | `width:100%;` 无居中容器 | 系统架构、相机标定流程 |
| **左右分栏** | 对比/对照/AB 选型 | `display:flex; flex-direction:row;` | 绝对 vs 相对、JPEG vs RAW |
| **网格卡片** | 多项并列（设备/概念/案例） | `display:grid; grid-template-columns:repeat(...)` | 三件神器、四件武器 |
| **表格矩阵** | 参数对照/SOP步骤/采集方案 | CSS Grid 模拟表格 或 `<table>` | 光照矩阵、采集矩阵表 |
| **非对称分栏** | 主图+侧边信息（60/40, 70/30） | `grid-template-columns: 3fr 2fr` | 左大图右说明 |
| **Z 字形/对角线** | 时间线/步骤流程 | `transform:skew()` / 交替左右排列 | 处理流水线 |
| **放射/焦点** | 中心概念向外发散 | `position:absolute` + `transform` | 核心能力雷达 |
| **沉浸全屏** | 单一视觉主体（场景/图表） | 全屏背景 + 叠加文字 | 工厂车间场景、案例现场 |

> **重要**：以上 8 种模式是最小基础集。可混合使用（如左分栏+右网格），但**绝对禁止**连续两章使用同一布局模式。每段（S1-S6）至少出现 3 种不同布局。
...
8. **顶部留空 50-70px + 底部留空 60-80px** — layout 上不贴边下不叠字幕。禁止用 `padding` 缩写覆盖 scene-pad，如需增加水平边距使用独立的 `padding-left/right`
9. **JSX 文本内容禁止 `\u` 转义** — JSX 标签间的文本按 HTML 解析，`\u` 无效。直接写实际 UTF-8 字符或用 `{'\u...'}` 表达式。引号内 JS 字符串不受限。
10. **禁止全部章节使用同一种布局** — 每章根据内容类型选择不同构图模式（全版/分栏/网格/表格/非对称/Z字形/放射/沉浸）。连续两章不得同模式，每段至少出现 3 种模式。
