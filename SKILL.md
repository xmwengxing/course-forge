---
name: course-forge
description: 把知识点、知识文档或口播稿，做成带旁白配音、互动测验、嵌入式评估和可部署到应用的分段式互动课件。产出物 = Vite + React + TS 项目 + course.json 三级课程架构 + 按章节切分的音频 + 字幕时序。工作流：知识文档 → 分段拆分(S1-S5) → 逐段开发(每段3-8章) → 互动组件嵌入(选择题/简答题/CSS 3D) → 音频合成(provider-agnostic) → 字幕生成 → 部署嵌入。适合职业教育、企业培训、技能认证等场景。
---
...
**本 Skill 基于 web-video-presentation 的成熟方法论，在此基础上增加了课程模式（S1-S5 分段、互动评估、部署嵌入）。**
...
| | 单视频 (WVP) | 课程模式 (CF) |
|:--|:--|:--|
| 时长 | 1 个视频 (~3-15 min) | 1 门课 (~45-60 min) |
| 结构 | 章节线性排列 | S1-S5 分段，每段 3-8 章 |
| 评估 | 无 | 弹题/简答/课后挑战 + 柯氏四级 |
| 部署 | 录屏输出 | 嵌入 Web 应用 + DB 驱动导航 |
| 使用 web-video-presentation | **直接用** | 不要用——用本 Skill |
