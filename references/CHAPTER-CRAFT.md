...
> 整页内容全部展示后至少有 1 处可点击、可拖拽或可切换的交互点。整页内容零交互 = 验收不过 = 回去重做。
...
内容揭示的每一步，元素可以从任何位置入场（左 / 中 / 右 / 上 / 下）——
不强制从上到下。讲者视角是"我在讲哪个位置的事，元素就出现在哪"，
元素和它讲述的位置天然对齐。

**最终画面的视觉中心必须是画布中心**：

- 哪怕左上角元素和右下角元素关联互动（如箭头 / 流程线），
  二者连成的"视觉重心"也要落在画布中心
- 元素**不必**方方正正垂直 / 水平居中；视觉重心落在画布中点 = 通过
- 比例失衡的画面（左重右轻 / 上重下空 / 单角堆满）= 验收不过

> **判断方法**：从画布 4 角画对角线，4 条对角线交于画布正中心。
> 把所有重要元素（文字 / 图形 / 标线 / 卡片）的几何中心做加权平均，
> 看平均点是否落在这 4 对角线交点附近 ±20% 画布宽 / 高内。
> 落 = 通过；偏离过远 = 回去重排元素位置或调整容器对齐。
...
章节根容器的 CSS 必须**同时具备**这 5 条，缺一就溢出：

```css
.chapter-root {
  display: flex;              /* 或 grid，按内容选 */
  flex-direction: column;
  min-height: 100%;          /* 撑满舞台；内容不足时画布仍占满 */
  max-width: 1840px;         /* 关键：防超出 1920 视口被 scale 截断 */
  margin: 0 auto;            /* 关键：水平居中 */
  padding: 32px 48px;        /* 默认 4 边安全区（≥80px 等同边界）*/
  overflow: hidden;          /* 关键：超出直接截，不撑开 */
  gap: 14px;
}
```

- 缺 `max-width:1840px` → 内容超出 1920 视口，会被 transform scale 放大截断
- 缺 `overflow:hidden` → 单个 card 过高会把内容推到屏外
- 缺 `margin:0 auto` → 内容偏左/右
- **`min-height:100%` + 内容自由呼吸**：撑满舞台保留大留白，元素在内部
  可左可右可居中，由内容逻辑决定 —— 但**最终视觉重心必须落画布中心**
  （参见上方「视觉中心」段）。不要为了"垂直居中"强行加 `justify-content:center`，
  反而会破坏内容叙事位置（左上的元素讲左上，右下的元素讲右下）。

### flex/grid 容器底线（**容器内子节点的硬约束**）

如果章节用了 `display:flex` 或 `display:grid`，**子节点必须有**：

```css
.chapter-child {
  display: flex;             /* 或 block / grid */
  flex-direction: column;
  min-height: 0;             /* 关键：让 child 在 flex 父容器内可被压缩 */
  flex-shrink: 0;            /* 内容过大时改 min-height: 0; overflow: auto; */
  overflow: hidden;          /* 或 auto，看内容是否真要滚动 */
}
```

- 缺 `min-height:0` → flex 子节点默认 min-height: auto → 内容会撑开父容器
- 父 flex 容器**必须**给所有 child `min-height:0`，否则会越撑越高

grid 容器的子节点**总宽 ≤ 父容器宽**：

```css
.chapter-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
/* 两列 1fr+1fr 在 1840px 容器中 = (1840-48-12)/2 = 890px/列; */
/* 在 1280px 容器中 = (1280-48-12)/2 = 610px/列; 不会溢出。 */
```

> **判断标准**：在 Chrome DevTools Responsive 切到 1920×1080 和 1280×720，**两种视口**下内容完整且不被裁切 = 通过。
...
- [ ] **根容器 5 件套齐备**（`display:flex + min-height:100% + max-width:1840px + margin:0 auto + overflow:hidden`）—— 任一缺 = 溢出风险
- [ ] **flex/grid 子节点有 `min-height:0`** —— 默认 `min-height:auto` 会撑开父容器
- [ ] **Chrome DevTools Responsive 切 1920×1080 验证**：内容完整显示、不被裁切
- [ ] **Chrome DevTools Responsive 切 1280×720 验证**：letterbox 允许，但内容不能被裁切后产生空白带
...
每章汇报完成时**必须**附上 4 件事，让用户能快速验收：

```
第 <N> 章 <id> 做完了。

1. 验收链接（dev server 跑起来时用）：
   http://localhost:5174/?course=<course-id>&auto=0
   （录屏模式：+&auto=1）

2. 期望看到的视觉气质：<与主题 bestFor / mood 的匹配说明>

3. 重点验收的 step（最容易踩坑的 1-2 步）：
   - Step <K>：<说明，例如"漏白边指示箭头要贴合 4 个角" >
   - Step <K>：<说明>
