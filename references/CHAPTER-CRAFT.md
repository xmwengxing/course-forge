...
> 每 3 章至少有 1 处**真交互**。3 章内无交互 = 验收不过 = 回去重做。
>
> ⚠️ **"真交互"标准** —— 全部满足才合格：
...
**怎么组合发挥都行 —— 但每3章必须用，不允许连续3章0互动0交互。**
...
- [ ] **每 3 章（chapter count）内必须含 ≥ 1 处真交互点**（onClick/onChange/drag 触发 useState 状态变化）—— **不数**hover 动画/transition/章节导航/进度条/翻页器。判定方法：关掉所有 `pointer-events: none`，屏上是否有元素能点击且真的 setState？无 = 回去加（§ 必须用交互 详述）
