...
`---` 之间的内容就是后面 `outline.md` 里的一个 step。**步号由 `---` 顺序隐式决定**（第 1 个 `---` 块 = step 0，0-indexed），**不要**显式标 `### step N` 标题/序号——`script.md` 仅含口播文本 + `---` 分隔（`OUTLINE-FORMAT.md` 自检要求"脚本不得包含标题、序号等非口播内容"）。开发时 `narrations.ts` 数组顺序与 `---` 块顺序一一对应。
