...
> ⚠️ **本文件默认要读**（不是"主模式单视频"）。
> SKILL.md 流程总图 Phase 0 是新加的"模式判断"步骤——读本文件后, 决定走 [单视频] 还是 [课程模式]。

本文件记录 Course Forge **相较于单视频模式的增量能力**：
课程结构 / 课程结构文档 / 互动评估 / 多课程管理 / 故障自检。
**默认"课程模式"** (除非用户明确说"只做单视频")。

## 模式判断 checklist (Phase 0 必走)

读完源文档路径, 用这 3 个问题判断走哪种模式。**任意一个 = 课程模式**。
...
如果 ≥ 1 命中 → **课程模式**, 必做:
- 生成 `course.json` 三级结构 (课/段/章)
- 启用 ChapterMenu / ModeControls / QuizPanel 等组件
- 末段加柯氏四级评估 (L1 反应 + L2 学习)

如果 0 命中 → **单视频模式**, 走 SKILL.md 主流程即可。

> **反例警告**:
> SKILL.md 早期版本第一句写"主模式是单视频, 本文件默认不读",
