...
> **为什么 3 章**：3 章 = 33% 屏带交互的近似平衡点 — 低于 3 章出现 1 次交互
> 太密集（打断叙述节奏），高于 5 章又太松（用户连续多屏没机会动手）。
> skill **不约束**交互的具体时机（与每章步数 / 单步口播长度解耦），只约束
> 课程级密度 = 每 3 章至少出现 1 次。
...
下面 6 个反模式是**实际项目跑出来的真实坑**（写于 2026-07），**写课程时必看必避**。
这些规则跨领域通用，**不**针对任何特定课件。
