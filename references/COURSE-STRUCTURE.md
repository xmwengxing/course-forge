...
| **大纲·分段 Outline Segment（原 S1~S5）** | 一级导航菜单项。两种尺度：**(a) 短视频默认**——按内容主题划分的块（导入/精讲/案例/收官…），段数依内容；**(b) 长课模式**——一节课 Lesson（~60min）。段数 = 课时量 | `course.json` 的 segment 条目（目标模型） |
| **章节 Chapter（章 / 课）** | 二级导航菜单项。两种尺度：**(a) 短视频**——30~60s 教学微单元；**(b) 长课**——小节主题（1 屏或多屏），如「1.1 你好，数字世界！」。一个章节由 1 屏或多屏组成 | `course.json` 章节条目 + `chapters.ts` 注册 |
...
### 1.1 两种尺度语义（同一套两级结构，两种装法）

本 skill 默认是**短视频基因**（一章 30~60s）。但当课程进入"每节课数十分钟"的长课尺度时，
**不用改一行 runtime 代码**，只要把两级结构的语义重映射到长课尺度即可：

| 运行时结构（不变） | 短视频语义（默认） | **长课语义（大课 / 需分工）** |
|---|---|---|
...
- 长课的口播稿撰写 / 子代理分工流程见 [`SCRIPT-WRITING.md`](SCRIPT-WRITING.md)。

> ⚠️ **"分段"一词专指一级**（大纲·分段）。画面开发/验收的**增量单元请说"逐小节"**（= 二级章节），
> 不要说"分段开发"——否则与一级混淆，重蹈 `section/segment/chapter` 混用的坑。
> 即：口播稿阶段**逐"节课"（一级）**分配子代理；画面阶段**逐"小节"（二级）**增量开发 + 验收。
