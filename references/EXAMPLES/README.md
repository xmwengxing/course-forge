...
> **正确使用流程**：
>
> 1. 走完 [`../CHAPTER-CRAFT.md`](../CHAPTER-CRAFT.md) § 这是视频，不是 PPT + § 视觉中心：舞台中心 ≠ 元素居中
...
> § 避免 AI 味 列出的同质化反模式（每个用户的视频看起来像同一个模板的
> N 个变奏）。
...
§ 这是视频，不是 PPT 第 7 条节制使用）、真素材（不是占位卡）、
字号狠对比、绑了 `newsroom` 主题作为示范。
...
§ 字体 / 配色 / 动画 / 留白 —— 视频演示基本审美（CSS / SVG / Canvas / JS 全栈）里覆盖了；按 anchor
的"形"组合即可。
